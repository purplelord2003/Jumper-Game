...
class Obstacle(pygame.sprite.Sprite):
    ...
    def __init__(self, type):
        super().__init__()
        self.type = type
        if type == 'ice' or type == 'ice_when_lava':
        ...
        if type == 'default1':
            ...
            self.rect = self.image.get_rect(bottomleft = (randint(0, 504), -150))
    
        elif type == 'default4':
            self.rect = self.image.get_rect(bottomleft = (randint(0, 504), -400))

        elif type == 'lava':
            self.rect = self.image.get_rect(bottomleft = (randint(0, 204), -400))
        
        elif type == 'platform_when_lava':
            self.rect = self.image.get_rect(bottomleft = (randint(300, 504), -400))
        
        elif type == 'ice_when_lava':
            self.rect = self.image.get_rect(bottomleft = (randint(0, 504), -275))
    ...
    def update(self):
        ...
        if self.type == 'ice' or self.type == 'ice_when_lava':
        ...
        for i in collision_list:
            if i.type == 'lava':
                ...
                if player.sprite.rect.clipline((i.rect.left + 10, i.rect.top), (i.rect.right - 5, i.rect.top)) and player.sprite.gravity >= 0:
                    ...
                    if i.type == 'ice' or i.type == 'ice_when_lava':
        ...
        if game_active:
            ...
            if event.type == obstacle_timer:
                ...
                if obstacle_chosen == 'lava':
                    ...
                    obstacle_group.add(Obstacle('ice_when_lava'))
                else:
            ...
            if event.type == speed_timer:
                ...
                pygame.time.set_timer(obstacle_timer, int(4167/screen_down))
                if screen_down == 2:
                    stage1_music.stop()
            ...
            if event.type == pygame.KEYDOWN and event.key == pygame.K_SPACE:
                ground_rect = ground_surf.get_rect(topleft = (0, 500))
                pygame.time.set_timer(obstacle_timer, 4167)
                pygame.time.set_timer(speed_timer, 20800)
                game_active = True
                player.add(Player())
                max_score = 0.01
                screen_down = 1
